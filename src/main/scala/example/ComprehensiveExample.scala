...
object ComprehensiveExample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val users = (sc.textFile("C:\\Users\\kmyko\\Desktop\\computer architecture\\lab\\src\\test\\users.txt")
      .map(line => line.split(",")).map( parts => (parts.head.toLong, parts.tail) ))

    // Parse the edge data which is already in userId -> userId format
    val followerGraph = GraphLoader.edgeListFile(sc, "C:\\Users\\kmyko\\Desktop\\computer architecture\\lab\\src\\test\\followers.txt")
...
//    println(userInfoWithPageRank.vertices.collect().mkString("\n"))
    // $example off$
